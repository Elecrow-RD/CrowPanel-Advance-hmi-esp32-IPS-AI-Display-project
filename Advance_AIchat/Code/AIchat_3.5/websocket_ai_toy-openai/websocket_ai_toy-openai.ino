...
void init_screen() {
  ...
  TFT.setRotation(0);         // This display orientation is correct. Set it to 1.
...
void change_status() {
  if (!isOpenMic && !isOpenSpk) {
    ...
    TFT.pushImage(0, 0, 480, 320, (uint16_t*)img);//(starting x-coordinate, starting y-coordinate, image length, image width). The image length and image width can be viewed by right-clicking on the image and selecting "Properties"
  } else if (isOpenMic && !isOpenSpk) {
    // Display the listening icon
    TFT.pushImage(120, 37, 241, 247, (uint16_t*)listen);
  } else if (!isOpenMic && isOpenSpk) {
    // Display the speaking icon
    TFT.pushImage(120, 37, 241, 247, (uint16_t*)speak);
  }
}